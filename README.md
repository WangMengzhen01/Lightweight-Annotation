...
为了响应同行评审关于可复现性的建议，我们对本仓库进行了重大更新：
统一训练框架：通过配置开关，一套代码即可复现 Baseline (U2Net) 和改进模型 (U2Net++)。
完整预处理流程：公开了从 Labelme 标注到训练集生成的完整处理细节。
轻量化标签生成：新增独立的 DP_BAS_Label.py 脚本，支持通过配置开关（DP/BAS）来独立观测 Douglas-Peucker 简化与 BAS 边界优化的具体作用。
简介
